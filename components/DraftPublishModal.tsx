...
const DraftPublishModal: React.FC<Props> = ({ open, onClose, authService, availableClasses, draftTitle, draftId, keepDraftDefault, keepDraftLocked, onPublished }) => {
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 z-[90] flex items-center justify-center p-4">
        <div className="bg-white border-4 border-brand-brown rounded-3xl w-full max-w-xl shadow-comic">
          <div className="p-6 border-b-4 border-brand-brown bg-[#C0E2BE] flex items-center justify-between">
            <div className="text-2xl font-black text-brand-brown">派發草稿</div>
            <button
              onClick={onClose}
          ...
          <div className="p-6 font-bold text-gray-700">未有班別資料（availableClasses 為空）。</div>
        </div>
      </div>
    );
  }
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-[90] flex items-center justify-center p-4">
      <div className="bg-white border-4 border-brand-brown rounded-3xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-comic">
        <div className="p-6 border-b-4 border-brand-brown bg-[#C0E2BE] flex items-center justify-between">
          <div>
            <div className="text-2xl font-black text-brand-brown">派發草稿</div>
            <div className="text-sm text-brand-brown/80 font-bold">{draftTitle}</div>
          </div>
          <button
            onClick={onClose}
            className="w-10 h-10 rounded-full bg-white border-2 border-brand-brown hover:bg-gray-100 flex items-center justify-center"
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <button
              onClick={async () => {
                ...
                setError('');
                try {
                  const resp = await authService.publishDraft(draftId, { className, classFolderId: resolvedFolderId, ...(keepDraftLocked ? {} : { keepDraft }) });
                  alert('派發成功！');
                  onPublished?.(resp);
                  onClose();
                } catch (e: any) {
              ...
              disabled={publishing || loadingFolders}
            >
              {publishing ? '派發中…' : '派發'}
            </button>
          </div>
